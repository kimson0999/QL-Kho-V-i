...
if($query->execute()){
echo '<script language="javascript">alert("Thêm thành công"); window.location="nhanvien.php";</script>';
}
}
//LOGIN
...
if(isset($_POST['cn'])){
	...
	$kt=$conn->query("SELECT email from users where id='$ma'");
	$ktmail=$kt->fetchAll(PDO::FETCH_ASSOC);
	foreach ($ktmail as $key => $value) {
	
	if($email==$value['email']){
		echo '<script language="javascript">alert("Email bị trùng !"); window.location="profile.php";</script>';
	}
	else
	{
		
		$sql2="UPDATE users set email='$email',fullname='$ten', birthday='$ns',phone='$sdt',address='$dc',updatedate='$date' where id='$ma'";
